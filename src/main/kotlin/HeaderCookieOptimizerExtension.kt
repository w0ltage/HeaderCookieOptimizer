...
class HeaderCookieOptimizerExtension : BurpExtension, ContextMenuItemsProvider {
    ...
    override fun provideMenuItems(event: ContextMenuEvent): List<Component> {
        val messageEditor = event.messageEditorRequestResponse().orElse(null) ?: return emptyList()
        val menuItem = JMenuItem("Optimize Headers & Cookies").apply {
            addActionListener {
                startOptimization(messageEditor)
            }
        }
        return listOf(menuItem)
    }

    private fun startOptimization(messageEditor: MessageEditorHttpRequestResponse) {
    ...
    private fun optimizeRequest(
        ...
        SwingUtilities.invokeLater { messageEditor.setRequest(optimizedRequest) }
